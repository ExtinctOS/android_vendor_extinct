...
func init() {
	android.RegisterModuleType("extinct_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = extinctExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = extinctExpandVariables(ctx, dep_file)
	...
	cmd := extinctExpandVariables(ctx, String(g.properties.Cmd))
